...
public class MultiTest {
	...
	private static String start(final Class<? extends Object> clazz, Object obj)
			throws InterruptedException, InstantiationException, IllegalAccessException {
		String result = null;
		...
		if (clazz.isAnnotationPresent(ConcurrentTest.class)) {
			...
			for (Method method : clazz.getDeclaredMethods()) {
				...
				if (method.isAnnotationPresent(NewThread.class)) {
					...
					int howMany = newthread.howMany();

					threadlist.add(newthread);
					methodlist.add(method);
					howManylist.add(howMany);

					howManyThreadsTtl += howMany;
			...
			for (int i=0,t=0, max=methodlist.size(); i < max; i++) {
			...
			result = String.format("=== %50s done %,d time(s) in %5.1f %3s (%5.1f %3s/try) === ", clazz.getSimpleName(),
					maxtry, UtilTimer.timeValMls(ttlmls), UtilTimer.timeScaleMls(ttlmls),
			...
			throw new RuntimeException(String.format("No annotation @%s presented for class %s",
					ConcurrentTest.class.getName(), clazz.getName()));
		}
